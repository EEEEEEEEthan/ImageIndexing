...
namespace ImageIndexing
{
    internal class Program
    {
        ...
        private static readonly LLMClient client;
        ...
        private static async void Start(Action callback)
        {
            try
            {
        ...
        private static async Task<(bool success, string result)> Summary(string filePath)
        {
            var success = false;
            string result = null;
            for (var i = 0; i < 3; i++)
